...
<form onSubmit={handleSubmit}>
  ...
  <div className="mb-6">
    <label htmlFor="submerchantuserexpdeglab" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Sub-merchant User Experience designs</label>
    <div className="flex items-center justify-center w-full">

    <label htmlFor="submerchantuserexpdeg" className="flex flex-col items-center justify-center w-full h-[180px] border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
          {selectedFile ? (
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="mb-2 border border-slate-300 rounded-full p-1 text-sm text-gray-500 dark:text-gray-400"><TbFileUpload className='inline-block mr-2 text-2xl' />{selectedFile.name} <IoMdCloseCircle className='ml-2 inline-block opacity-70 text-xl'  onClick={handleRemoveFile}/></p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              <svg className="w-8 h-8 mb-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16">
            ...
            <p htmlFor="submerchantuserexpdeg" className='bg-gray-500 pb-[6px] pt-1 text-sm px-4 font-semibold rounded-full text-white hover:bg-primary duration-100'>Upload</p>
            </div>
          )}
          <input
            required
...
<small className='text-[10px]'>If your Sub-Merchant is designing their own User Experience (UX) for PayID, please attach screenshots of this UX</small>
  </div>

  <div className="mb-6">
    <label htmlFor="othercommentsreachtxt" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Other comments</label>
    ...
    <div className="mb-6 hidden" >
          ...
          <input onChange={handleFillChange} name="iffillformone" value={formData.iffillformone} type="text" id="iffillformone" className="bg-gray-50 hidden border-gray-300 text-gray-900 w-full p-2.5 text-sm rounded-lg outline-none border-2 focus:border-primary " placeholder="Company/Legal Entity Name of the Sub-merchant" />

      
    </div>

  <div className='flex items-center mt-16 py-4 gap-5'>
